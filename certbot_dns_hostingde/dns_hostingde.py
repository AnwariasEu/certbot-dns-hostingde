...
class Authenticator(dns_common_lexicon.LexiconDNSAuthenticator):
    ...
    description = 'Obtain certificates using a DNS TXT record ' + \
                  '(if you are using Hosting.de Infrastructure Service for DNS).'

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
        self._add_provider_option('api-token',
                                  'API token for Hosting.de Infrastructure Service '
    ...
    def add_parser_arguments(cls, add: Callable[..., None],
        ...
        add('credentials', help='Hosting.de Infrastructure Service credentials file.')

    def more_info(self) -> str:
        return 'This plugin configures a DNS TXT record to respond to a dns-01 challenge using ' + \
               'the Hosting.de Infrastructure Service API.'
